...
from clawphd import __version__, __logo__
...
def onboard():
    ...
    from clawphd.config.loader import get_config_path, save_config
    from clawphd.config.schema import Config
    from clawphd.utils.helpers import get_workspace_path
...
I am clawphd, a lightweight AI assistant.
...
def gateway(
    ...
    from clawphd.config.loader import load_config, get_data_dir
    ...
    if not api_key and not is_bedrock:
        ...
        console.print("Set one in ~/.clawphd/config.json under providers.openrouter.apiKey")
        raise typer.Exit(1)
...
def _build_diagram_providers(config):
    ...
    for candidate in [
        Path(__file__).resolve().parent.parent.parent / "references",
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".clawphd" / "bridge"
...
def cron_list(
    ...
    """List scheduled jobs."""
    from clawphd.config.loader import get_data_dir
    from clawphd.cron.service import CronService
